...
#battleView .battle-skill {
  ...
  box-shadow:
    ...
    0 12px 28px rgba(2,6,23,.2) !important;
...
#battleView .battle-skill:has(button:not(:disabled)) {
  ...
  box-shadow:
    ...
    0 14px 32px rgba(49,46,129,.18) !important;
}
...
#battleView .battle-skill:has(button:not(:disabled)):hover {
  ...
  box-shadow:
    ...
    0 18px 34px rgba(49,46,129,.24) !important;
}
...
#battleView .battle-skill button:disabled {
  cursor: default;
  opacity: 1 !important;
}
...
#battleView .battle-skill-icon {
  ...
  border-radius: .82rem;
  background:
    radial-gradient(circle at 50% 35%, rgba(165,180,252,.15), transparent 70%),
    rgba(15,23,42,.52);
...
#battleView .battle-skill-icon img {
  width: 100%;
  height: 100%;
  object-fit: contain;
}
...
#battleView .battle-skill:has(button:not(:disabled))::after {
  content: 'BEREIT';
  color: #a7f3d0;
}

#battleView .battle-skill.skill-locked::after {
  content: 'COOLDOWN';
  color: #c4b5fd;
}

#battleView .battle-skill.used::after {
  content: 'BENUTZT';
  color: #94a3b8;
}
...
#battleView .battle-skill.used .battle-skill-name {
  color: #cbd5e1 !important;
}
...
@media (max-width: 620px) {
  ...
  #battleView .battle-skill-desc {
    min-height: 40px;
    padding: .48rem .7rem .58rem !important;
  }
}
'''
...
qa_path = Path('.github/workflows/tk2-2026-qa.yml')
qa = qa_path.read_text(encoding='utf-8')
if 'node --check 2026/qa/battle-ui-browser.mjs' not in qa:
    qa = qa.replace('          node --check 2026/qa/dev-overlay-browser.mjs\n', '          node --check 2026/qa/dev-overlay-browser.mjs\n          node --check 2026/qa/battle-ui-browser.mjs\n', 1)
if 'test -f 2026/qa/battle-ui-browser.mjs' not in qa:
    qa = qa.replace('          test -f 2026/qa/dev-overlay-browser.mjs\n', '          test -f 2026/qa/dev-overlay-browser.mjs\n          test -f 2026/qa/battle-ui-browser.mjs\n', 1)
if "grep -q 'BATTLE UI SKILL BAR PREMIUM PASS 2026' 2026/modern-battle.css" not in qa:
    qa = qa.replace("          grep -q 'SINGLE ARENA MOBILE OVERFLOW GUARD' 2026/modern-battle.css\n", "          grep -q 'SINGLE ARENA MOBILE OVERFLOW GUARD' 2026/modern-battle.css\n          grep -q 'BATTLE UI SKILL BAR PREMIUM PASS 2026' 2026/modern-battle.css\n", 1)
if 'Battle UI skill bar gate' not in qa:
    qa = qa.replace('      - name: Economy browser gate\n', '      - name: Battle UI skill bar gate\n        run: node 2026/qa/battle-ui-browser.mjs\n\n      - name: Economy browser gate\n', 1)
