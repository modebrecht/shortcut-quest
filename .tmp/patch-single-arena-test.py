...
start = s.index('  const threePanelLayout = await page.evaluate(() => {')
end = s.index('  const previewArenaBackground = await page.locator', start)
new = '''  const singleArenaLayout = await page.evaluate(() => {
...
'''
s = s[:start] + new + s[end:]
s = s.replace("  assert.equal(/battle-bg-forest/i.test(previewArenaBackground), false, 'Arena preview must use the dark stone arena instead of the forest background');", "  assert.match(previewArenaBackground, /arena-premium\\.jpg/i, 'Arena preview must use the premium generated arena asset');\n  assert.equal(/battle-bg-forest/i.test(previewArenaBackground), false, 'Arena preview must never use the forest background');", 1)
s = s.replace("  assert.equal(/battle-bg-forest/i.test(activeArenaBackground), false, 'Active combat must not switch back to the forest background');", "  assert.match(activeArenaBackground, /arena-premium\\.jpg/i, 'Active combat must use the same premium arena asset');\n  assert.equal(/battle-bg-forest/i.test(activeArenaBackground), false, 'Active combat must not switch back to the forest background');", 1)
